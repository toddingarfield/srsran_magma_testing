...
def get_ip_address():
    ...
    s.connect(('8.8.8.8', 32520))  # connecting to a UDP address doesn't send packets
...
def main():
    ...
    parser.add_argument('-i', '--ipaddr', type=str, default="")
    ...
    httpd = socketserver.TCPServer((args.ipaddr, port), Handler)
    if not args.ipaddr:
        listen_ip_addr = get_ip_address()
    else:
        listen_ip_addr = args.ipaddr
    print("Watch stream at: http://{}:{}/playlist.m3u8".format(listen_ip_addr, port))
    try:
        httpd.serve_forever()
    except:
        httpd.server_close()
